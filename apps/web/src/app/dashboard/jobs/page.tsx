...
import { Sparkles, Clock, CheckCircle, XCircle, Loader2, Eye, RefreshCw } from 'lucide-react';
...
export default function JobsPage() {
  ...
  const getJobHref = (job: Job) =>
    job.jobType === 'video'
      ? `/dashboard/video?job=${job.id}`
      : `/dashboard/generate?job=${job.id}`;
          ...
          {jobs.map((job) => (
            <div key={job.id} className="glass-card-hover p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                {job.prompt && <p className="text-xs text-white/40 mt-1 truncate">{job.prompt}</p>}
              </div>
              ...
              <Link href={getJobHref(job)} className="btn-ghost p-2">
